...
from .forms import ContatoprofessorForm
...
@has_permission_decorator('visualizarContatoProfessor')
def listar(request):
    if request.method == 'GET':        
        contatos = Contatoprofessor.objects.all()  # Obtém todos os contatos de professores do banco de dados
        formContato = ContatoprofessorForm()  # Instancia o formulário para criar novos contatos
...
@has_permission_decorator('adicionarContatoProfessor')
def criar(request):
    if request.method == 'POST':  # Verifica se a solicitação é POST
        form = ContatoprofessorForm(request.POST)  # Instancia o formulário com os dados da solicitação
...
@has_permission_decorator('editarContatoProfessor')
def editar(request, contato_id):
    if request.method == 'POST':  # Verifica se a solicitação é POST
        contato = Contatoprofessor.objects.get(pk=contato_id)  # Obtém o contato pelo ID
        form = ContatoprofessorForm(request.POST, instance=contato)  # Instancia o formulário com os dados da solicitação e o contato existente
        if form.is_valid():  # Verifica se o formulário é válido
...
@has_permission_decorator('excluirContatoProfessor')
def excluir(request, contato_id):
    if get_object_or_404(Contatoprofessor, pk=contato_id):  
        contato = get_object_or_404(Contatoprofessor, pk=contato_id)# Obtém o contato pelo ID ou retorna 404 se não encontrado
